...
struct Payloads {
  ...
  transactionIndex @1 :UInt8;
  logIndex @2 :UInt16;

  union {
    transfer @3 :Transfer;
    delegateChanged @4 :DelegateChanged;
    delegateVotesChanged @5 :DelegateVotesChanged;
  }
...
def safe_int(d, k):
    return int(d[k]) if k in d and d[k] is not None else 0

def cast_address(address):
    return bytes.fromhex(address[2:])  # strip "0x" and hex-decode

def parse_event(row, event_type, msg):
    if event_type == "transfer":
        msg.transfer = msg.init("transfer")
        msg.transfer.fromAddress = cast_address(safe_get(row, "from"))
        msg.transfer.toAddress = cast_address(safe_get(row, "to"))
        msg.transfer.value = safe_get(row, "value")

    elif event_type == "delegateChanged":
        msg.delegateChanged = msg.init("delegateChanged")
        msg.delegateChanged.delegator = cast_address(safe_get(row, "delegator"))
        msg.delegateChanged.fromDelegate = cast_address(safe_get(row, "from_delegate"))
        msg.delegateChanged.toDelegate = cast_address(safe_get(row, "to_delegate"))

    elif event_type == "delegateVotesChanged":
        msg.delegateVotesChanged = msg.init("delegateVotesChanged")
        msg.delegateVotesChanged.delegate = cast_address(safe_get(row, "delegate"))
...
if DO_WRITE:
    ...
    with open("events.capnp", "w+b") as f:
...
cnt = 0
with open("events.capnp", "r+b") as f:
