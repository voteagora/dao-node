...
# Middleware to track end time and log it
@app.middleware("response")
async def log_request_time(request, response):
    total_time = time.time() - request.ctx.start_time
    request_time_ms = total_time * 1000  # Convert to milliseconds
    log_message = f"{request.method} {request.path} {response.status} " \
                  f"{request_time_ms:.2f}ms"
    # app.access_log.info(log_message)

@app.get("/balances")
async def balance(request):
    return html(f"Balances:  {app.ctx.balances}")


@app.route('/num_of_whales/<amount>')
async def num_of_whales(request, amount):
    ...
    for addr, value in app.ctx.balances.items():
        if value > amount:
            whales.append(addr)
            tot += 1

    return text(str(tot) + " whales with more than " + str(amount) + " DAOs.  Here are first 5: " + str(whales[:5]))

@app.route('/balance/<addr>')
async def balances(request, addr):
	return text(str(app.ctx.balances[addr]))
...
async def setup_background_task(app, loop):
    # Add the download_csv_from_gcs task
    app.add_task(download_csv_from_gcs(app))
    

if __name__ == "__main__":
    app.run(host="0.0.0.0", port=7654, dev=True, debug=True)
